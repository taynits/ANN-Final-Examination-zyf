...
import numpy as np
from models.Transformer import SwimTransformer
import timm
import  logging
...
for i in range(epochs):
    ...
    print('Epoch [{}] - Train - loss: {:.4f}  acc：{:.2f}%'.format(i + 1, avg_val_loss, val_accuracy*100))
    logging.info('Epoch [{}] - Train - loss: {:.4f}  acc：{:.2f}%'.format(i + 1, avg_val_loss, val_accuracy*100))

    if val_accuracy > best_acc:
        best_acc = val_accuracy
        torch.save(model.state_dict(), "results/model_Transformer_best.pth")
